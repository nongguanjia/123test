...
public class GotyeVoicePlayClickPlayListener implements View.OnClickListener,
	...
	public void stopPlayVoice(boolean byclick) {
		...
		isPlaying = false;
		((ChatActivity) activity).setPlayingId(-1);
	...
	public void playVoice(String filePath) {
		...
		((ChatActivity) activity).setPlayingId(message.getDbId());
		isPlaying = true;
	...
	public void onClick(View v) {
		if (((ChatActivity) activity).onRealTimeTalkFrom >= 0) {
			ToastUtil.show(activity, "正在实时通话中");
			return;
		}
		if (isPlaying) {
			if (((ChatActivity) activity).getPlayingId() == message.getDbId()) {
